...
//inutile (dato che il valore non viene modificato dalle procedure): in questo modo anche
//l'utilizzo complessivo è semplificato (ad esempio posso passare come buffer di memoria contenente
//la dll la property Memory di un oggetto di classe TStream o derivata)

//Italian to English translation
// I removed the var declaration of the first parameter of the functions in question because
// useless (because the value is not changed by the procedures): in this way also
// the total use is simplified (for example, I can pass as a memory buffer containing
// the dll, the memory property of an object of class TStream or derivative)
...
function BTMemoryLoadLibary(f_data: Pointer; const f_size: int64): PBTMemoryModule; stdcall;
// return value is nil if function fails
function BTMemoryGetProcAddress(f_module: PBTMemoryModule; const f_name: PAnsiChar): Pointer; stdcall;
// free module
procedure BTMemoryFreeLibrary(f_module: PBTMemoryModule); stdcall;
// returns last error
...
begin
  ...
  if (l_directory.Size > 0) then begin
    ...
    while (not IsBadReadPtr(l_importDesc, sizeof(TImageImportDescriptor))) and (l_importDesc.Name <> 0) do begin
      ...
      if (l_handle = INVALID_HANDLE_VALUE) or (l_handle = 0) then begin
        lastErrStr := 'BuildImportTable: can''t load library: ' + PAnsiChar(Cardinal(l_codeBase) + l_importDesc.Name);
        Result := false;
        exit;
      end;
